...
<div class="max-w-7xl mx-auto py-6 px-4">
    <!-- Tambahkan `items-start` di kelas grid di bawah ini -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="lg:col-span-2 bg-white border rounded-2xl p-6 shadow-sm lg:order-1">
            ...
            <form action="{{ route('attendance.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Masukkan Judul Absen" class="w-full border-gray-300 rounded-lg text-sm">
                </div>

                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Batas Waktu Pengisian (Kedaluwarsa)</label>
                    <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" required class="w-full border-gray-300 rounded-lg text-sm">
                </div>

                <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">PIC</label>
                        <input type="text" name="pic" value="{{ old('pic') }}" required placeholder="Nama penanggung jawab" class="w-full border-gray-300 rounded-lg text-sm">
                    </div>
                    <div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="tempat" required placeholder="Contoh: Ruang Adaptif, Lantai 2" class="w-full border-gray-300 rounded-lg text-sm">
                </div>

                <div class="mb-4">
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Rapat/Pertemuan</label>
                    <input type="text" name="rapat_pertemuan" required placeholder="Contoh: Rapat Koordinasi Mingguan" class="w-full border-gray-300 rounded-lg text-sm">
                </div>
        ...
        <div class="bg-white border rounded-2xl p-5 shadow-sm lg:order-2">
            <h3 class="font-bold text-gray-800 mb-4 text-base">
                {{ auth()->user()->role === 'admin' ? 'Seluruh Form Pengguna' : 'Riwayat Form Kehadiran Saya' }}
            </h3>
                    ...
                    <div class="p-3.5 border rounded-xl hover:border-emerald-500 transition bg-gray-50">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                @endif
                                @if($f->rapat_pertemuan || $f->tempat || $f->pic)
                                    <p class="text-[11px] text-gray-500 mt-1">
                                        {{ collect([
                                            $f->rapat_pertemuan,
                                            $f->tempat,
                                            $f->pic ? 'PIC: '.$f->pic.($f->divisi_pic ? ' ('.$f->divisi_pic.')' : '') : null,
                                        ])->filter()->implode(' • ') }}
                                    </p>
                                @endif
                                <p class="text-[11px] text-gray-500 mt-1">
                        ...
                        <div class="mt-3 flex gap-2 border-t pt-2.5">
                            @php
                                // Ganti IP:port di bawah ini sesuai alamat LAN server (tetap sama untuk semua form).
                                // Hanya bagian uuid yang berubah mengikuti masing-masing form.
...
function addField(label = '', type = 'text', req = false) {
    ...
    div.innerHTML = `
        <input type="text" name="fields[${fieldIdx}][nama]" value="${label}" placeholder="Nama Field (contoh: No. Telp)" required class="flex-1 text-sm border-gray-300 rounded-lg">
        <select name="fields[${fieldIdx}][tipe]" class="text-sm border-gray-300 rounded-lg">
        ...
        <label class="flex items-center gap-1 text-xs text-gray-600 px-2">
            <input type="checkbox" name="fields[${fieldIdx}][required]" value="1" ${req ? 'checked' : ''} class="rounded text-emerald-600"> Wajib
        </label>
        <button type="button" onclick="this.parentElement.remove()" class="text-red-500 hover:text-red-700 text-xs px-2 py-1">Hapus</button>
    `;
...
function copyLink(url) {
    ...
    // Saat diakses lewat IP LAN via HTTP biasa, browser menganggapnya "tidak aman"
    // sehingga navigator.clipboard bisa undefined dan gagal tanpa pesan error apa pun.
